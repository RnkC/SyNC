...
for i in range(1,n):
    ...
    Isyn1 = I(t[i-1],tt)
    # solve for next step
    z = odeint(model,z0,tspan,args=(u[i],Isyn1,noise[i]))
...
plt.plot(t[1000:2000],Isyn[1000:2000],'m-',label='Distortion')
...
plt.figure(figsize=(8.3,5.8))
#plt.plot(t[3600:4600],noise[3600:4600],'r-',label='Noise')
plt.plot(t[1000:2000],V[1000:2000],'r-',label='Distortion')
plt.yticks(fontsize=20)
plt.xticks(fontsize=20)
